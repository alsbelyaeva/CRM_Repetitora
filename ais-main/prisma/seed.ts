...
async function main() {
  ...
  await prisma.payment.createMany({
    data: [
      ...
      { clientId: annaClients[1].id, lessonId: lessons[6].id, amount: 3000, method: 'Перевод', dateTime: date(2026, 6, 3, 17, 10), note: 'Первый платеж июня по регулярной серии' },
      ...
      { clientId: sergeyClients[1].id, lessonId: lessons[12].id, amount: 5000, method: 'Перевод', dateTime: date(2026, 5, 8, 19, 45), note: 'Оплата мини-группы' },
      ...
      { clientId: sergeyClients[1].id, lessonId: lessons[16].id, amount: 5000, method: 'Перевод', dateTime: date(2026, 6, 5, 19, 45), note: 'Первая неделя июня, регулярная группа' },
