...
describe('Slot Requests API', () => {
  ...
  it('POST /api/slot-requests создает запрос слотов', async () => {
    const res = await request(app)
      ...
      .send({
        ...
        proposedSlots: [
          { from: '2026-07-18T10:00:00+03:00', to: '2026-07-18T11:00:00+03:00' },
          { from: '2026-07-18T12:00:00+03:00', to: '2026-07-18T13:00:00+03:00' },
        ],
      });
  ...
  it('PATCH /api/slot-requests/:id/restore восстанавливает запрос', async () => {
    await testPrisma.slotRequest.update({
      ...
      data: {
        status: 'CANCELLED',
        proposedSlots: [{ from: '2026-07-15T12:00:00+03:00', to: '2026-07-15T13:00:00+03:00', status: 'CANCELLED' }],
      },
    });
